...
class PurchaseController extends Controller
{
    public function purchaseOrder() {
        ...
        $requisitions = Requisition::where('status',1)->whereNotIn('id',$requisitionId)->get();
    ...
    public function purchaseOrderPost(Request $request) {
        ...
        $request->validate([
            ...
            'date' => 'required|date',
            'product.*' => 'required',
            'approved_requisition.*' => 'required|numeric|min:.01',
        ...
        $requisition = Requisition::where('id',$request->requisition)->first();
        ...
        foreach ($request->product as $reqProduct) {
            ...
            $productRequisition = ProductRequisition::where('requisition_id',$request->requisition)
                ->where('purchase_product_id',$request->product[$counter])
                ->first();
            $productRequisition->increment('purchase_quantity',$request->quantity[$counter] ?? 0);
            $productRequisition->decrement('purchase_due_quantity',$request->quantity[$counter] ?? 0);
        ...
        // dd($supplier);
        // $supplier->increment('total',$total);
        // $supplier->increment('due',$total);

        return redirect()->route('purchase_receipt.details', ['order' => $order->id]);
    }


    public function purchaseOrderEdit(PurchaseOrder $order){
        $products = PurchaseProduct::where("status",1)->get();
        $suppliers = Client::where('type',2)->where('status', 1)->orderBy('name')->get();
        $projects = Project::where('status', 1)->orderBy('name')->get();

        return view('purchase.purchase_order.edit', compact('suppliers','projects','order','products'));
    }
    ...
    public function purchaseReceipt() {
        ...
        return view('purchase.receipt.all',compact('warehouses'));
    }
    ...
    public function supplierPayment() {
        $suppliers = Client::where('type',2)->whereIn('project_type',[1,2])->get();
        // dd($suppliers);
    ...
    {
        return view('purchase.supplier_payment.supplier_payment_details',compact('client'));
    }
    ...
    public function purchaseInventoryDetails($project, $product) {
        ...
        return view('purchase.inventory.details', compact('product','project'));
    }
    ...
    public function utilizeAdd() {
        ...
        $warehouses = Warehouse::where('status', 1)
            ->orderBy('name')->get();
        return view('purchase.utilize.add', compact('warehouses','products','projects'));
    }
...
//        $log->save();
    ...
    public function orderReceive(Request $request) {
        ...
        foreach ($order->products as $product) {
            ...
            $exist = PurchaseInventory::where('warehouse_id',$request->warehouse)
                ...
                //->where('segment_id', $order->segment_id)
                ->where('purchase_product_id', $product->id)
                ->first();
            ...
            if ($exist) {
                ...
                $inventory->purchase_product_id = $product->id;
                $inventory->project_id = $order->project_id;
                //$inventory->segment_id = $order->segment_id;
           ...
           // $inventoryLog->segment_id = $order->segment_id;
            $inventoryLog->project_id =  $order->project_id;
    ...
    public function purchaseReceiptDatatable() {
        $query = PurchaseOrder::with('supplier','project','segment','requisition')->whereNotIn('purchase_type',[3]);
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('requisition_no', function(PurchaseOrder $order) {
                if ($order->requisition)
                return '<a target="_blank" href="'.route('requisition.details',['requisition'=>$order->requisition_id]).'" class="btn-link">'.$order->requisition->requisition_no.'</a>';
                else
                    return '';
            })
            ->addColumn('segment_name', function(PurchaseOrder $order) {
                return $order->segment->name ?? '';
            })
            ->addColumn('action', function(PurchaseOrder $order) {
                if ($order->received_at ==null && $order->paid ==0 )
                    return '<a href="'.route('purchase_receipt.details', ['order' => $order->id]).'" class="btn btn-primary btn-sm">View</a> <a class="btn btn-success btn-sm btn-receive" role="button" data-id="'.$order->id.'">Receive</a> <a href="'.route('purchase_order_edit', ['order' => $order->id]).'" class="btn btn-info btn-sm">Edit</a> <a role="button" data-id="'.$order->id.'" class="btn btn-danger btn-sm btn-delete">Delete</a>';
                elseif ($order->received_at !=null)
                    return '<a href="'.route('purchase_receipt.details', ['order' => $order->id]).'" class="btn btn-primary btn-sm">View</a> <a href="'.route('purchase_order_edit', ['order' => $order->id]).'" class="btn btn-info btn-sm">Edit</a>';
                else
                    return '<a href="'.route('purchase_receipt.details', ['order' => $order->id]).'" class="btn btn-primary btn-sm">View</a> <a href="'.route('purchase_order_edit', ['order' => $order->id]).'" class="btn btn-info btn-sm">Edit</a> <a class="btn btn-success btn-sm btn-receive" role="button" data-id="'.$order->id.'">Receive</a>';
            })
    ...
    public function estimateProductJson(Request $request) {
        if (!$request->searchTerm) {
            $products = ExtraCostProduct::where('status', 1)->orderBy('name')->limit(10)->get();
        } else {
            $products = ExtraCostProduct::where('status', 1)->where('name', 'like', '%'.$request->searchTerm.'%')->orderBy('name')->limit(10)->get();
        }
...
public function mobilizationProductJson(Request $request) {
        if (!$request->searchTerm) {
            $products = MobilizationWorkProduct::where('status', 1)->orderBy('name')->limit(10)->get();
        } else {
            $products = MobilizationWorkProduct::where('status', 1)->where('name', 'like', '%'.$request->searchTerm.'%')->orderBy('name')->limit(10)->get();
        }
    ...
    public function costingProductJson(Request $request) {
        if (!$request->searchTerm) {
            $products = ExtraCostProduct::where('status', 1)->orderBy('name')->limit(10)->get();
        } else {
            $products = ExtraCostProduct::where('status', 1)->where('name', 'like', '%'.$request->searchTerm.'%')->orderBy('name')->limit(10)->get();
...
//        dd($stock);
    ...
    public function supplierPaymentEdit(PurchasePayment $payment){
       // dd($payment);
        ...
        return view('purchase.supplier_payment.edit', compact( 'banks','payment','orders'));
