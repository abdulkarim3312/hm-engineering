    ...
    <script>
        $(function () {
            $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('project_loan.datatable') }}',
                columns: [
            ...
            $('body').on('click', '.btn-pay', function () {
                ...
                $.ajax({
                    method: "GET",
                    url: "{{ route('project_loan_payment.get_number') }}",
                    data: { projectId: projectId,loanType: loanType }
            ...
            $('#modal-btn-pay').click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('project_loan_payment.make_payment') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $('#modal-order').change(function () {
                ...
                if (loanId != '') {
                    $.ajax({
                        method: "GET",
                        url: "{{ route('project_loan_payment.order_details') }}",
                        data: { loanId: loanId }
                    }).done(function( response ) {
