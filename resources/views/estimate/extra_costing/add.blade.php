    ...
    <div class="col-md-12">
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">Extra Costing Information</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form method="POST" action="{{ route('extra_costing.add') }}">
                @csrf
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            @if (old('product') != null && sizeof(old('product')) > 0)
                                @foreach(old('product') as $item)
                                    <tr class="product-item">
                                        ...
                                        <td>
                                            <div class="form-group {{ $errors->has('unit_amount.'.$loop->index) ? 'has-error' :'' }}">
                                                <input type="text" class="form-control unit_amount" name="unit_amount[]" value="{{ old('unit_amount.'.$loop->index) }}">
                                            </div>
                                        </td>
                            ...
                            @else
                                <tr class="product-item">
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <input type="text" class="form-control unit_amount" name="unit_amount[]">
                                        </div>
                                    </td>
     ...
     <script>
         ...
         function calculate() {
             ...
             $('.product-item').each(function(i, obj) {
                 ...
                 var unit_amount = $('.unit_amount:eq('+i+')').val();
                 var quantity = $('.quantity:eq('+i+')').val();
 
                 if (cost_amount == '' || cost_amount < 0 || !$.isNumeric(cost_amount))
                     cost_amount = 0;

                 if (unit_amount == '' || unit_amount < 0 || !$.isNumeric(unit_amount))
                    unit_amount = 0;
         ...
         function initProduct() {
             $('.product').select2({
                 ajax: {
                     url: "{{ route('extra_work_product.json') }}",
                     type: "get",
                     dataType: 'json',
                     delay: 250,
