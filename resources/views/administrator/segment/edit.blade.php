                            ...
                            <div class="col-sm-10">
                                <select class="form-control select2" name="project">
                                    ...
                                    @foreach($projects as $project)
                                        <option value="{{$project->id}}" {{old('project',$project->id) == $segment->project_id ? 'selected':'' }}>
                                            {{$project->name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group {{ $errors->has('segment_unit') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control" placeholder="Enter Segment Unit"
                                       name="segment_unit" value="{{ empty(old('segment_unit')) ? ($errors->has('segment_unit') ? '' : $segment->segment_unit) : old('segment_unit') }}">
                        ...
                        <div class="form-group {{ $errors->has('segment_percent') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control" placeholder="Enter Segment Percent"
                                       name="segment_percent" value="{{ empty(old('segment_percent')) ? ($errors->has('segment_percent') ? '' : $segment->segment_percent) : old('segment_percent') }}">
