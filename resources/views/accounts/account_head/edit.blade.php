    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('account_head.edit', ['accountHead' => $accountHead->id]) }}">
                    @csrf
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" placeholder="Enter Account Head Name"
                                       name="name" value="{{ old('name',$accountHead->name) }}">
                        ...
                        <div class="form-group {{ $errors->has('account_code') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" placeholder="Enter Account Code"
                                       name="account_code" value="{{ old('account_code',$accountHead->account_code) }}">
                        ...
                        <div class="form-group {{ $errors->has('opening_balance') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" placeholder="Enter Opening Balance"
                                       name="opening_balance" value="{{ old('opening_balance',$accountHead->opening_balance) }}">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="type" id="type">
                                    ...
                                    @foreach($types as $type)
                                        <option value="{{ $type->id }}" {{ old('type',$accountHead->account_head_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                <span class="help-block">{{ $message }}</span>
