    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form method="POST" action="{{ route('requisition.approved',['requisition' => $requisition->id]) }}">
                    @csrf
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group {{ $errors->has('project') ? 'has-error' :'' }}">
                                    ...
                                    <select class="form-control project select2" style="width: 100%;" name="project" id="project" disabled>
                                        <option value="">Select Project</option>

                                        @foreach($projects as $project)
                                            <option value="{{ $project->id }}"
                                                {{ old('project',$project->id) == $requisition->project_id ? 'selected' : '' }}>{{ $project->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="form-group {{ $errors->has('date') ? 'has-error' :'' }}">
                                    ...
                                    <div class="input-group date">
                                        ...
                                        <input type="text" class="form-control pull-right" id="date" name="date"  readonly
                                               value="{{   date('Y-m-d',strtotime($requisition->date)) }}" autocomplete="off">
                                    </div>
                                    <!-- /.input group -->
                            ...
                            <div class="col-md-3">
                                <div class="form-group {{ $errors->has('note') ? 'has-error' :'' }}">
                                    ...
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="note" name="note" value="{{ $requisition->note}}" readonly>
                                    </div>
                                    <!-- /.input group -->
                            ...
                            <div class="col-md-3">
                                <div class="form-group {{ $errors->has('approved_note') ? 'has-error' :'' }}">
                                    <label>Approved Note</label>

                                    <div class="form-group">
                                        <input type="text" class="form-control" id="approved_note" placeholder="Enter Approved Note"
                                               name="approved_note" value="{{ old('approved_note')}}">
                                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th width="15%">Approved Quantity</th>
{{--                                    <th></th>--}}
                                </tr>
                                </thead>
                                        ...
                                        <tr class="product-item">
                                            <td>
                                                <div class="form-group {{ $errors->has('product.'.$loop->index) ? 'has-error' :'' }}">
                                                    <select class="form-control product" style="width: 100%;" name="product[]" required disabled>
                                            ...
                                            <td>
                                                <div class="form-group {{ $errors->has('quantity.'.$loop->index) ? 'has-error' :'' }}" readonly>
                                            ...
                                            <td>
                                                <div class="form-group {{ $errors->has('approved_quantity.'.$loop->index) ? 'has-error' :'' }}">
                                                    <input type="number" class="form-control approved_quantity" name="approved_quantity[]" value="{{ old('approved_quantity.'.$loop->index) }}">
                                                </div>
                                            </td>
...
{{--                                            </td>--}}
                                ...
                                @else
                                    ...
                                    <tr class="product-item">
                                        <td>
                                            <div class="form-group">
                                                <select class="form-control product" style="width: 100%;" name="product[]" required disabled>
                                                    ...
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->id }}" {{$product->id == $requisitionProduct->purchase_product_id ? "selected" : ''}}>{{ $product->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div class="form-group">
                                                <input type="text" name="unit[]" class="form-control unit" value="{{$requisitionProduct->unit->name}}" readonly>
                                            </div>
                                        </td>

                                        <td>
                                            <div class="form-group">
                                                <input type="number" class="form-control quantity" name="quantity[]"  value="{{$requisitionProduct->quantity - $requisitionProduct->approved_quantity}}" readonly>
                                            </div>
                                        </td>

                                        <td>
                                            <div class="form-group">
                                                <input type="number" class="form-control approved_quantity" name="approved_quantity[]"  value="{{$requisitionProduct->quantity - $requisitionProduct->approved_quantity}}"
                                                       max="{{$requisitionProduct->quantity - $requisitionProduct->approved_quantity}}">
                                            </div>
                                        </td>
...
{{--                                    </td>--}}
                                    ...
                                    <!--<th id="total-approved-quantity">  0.00 </th>-->
                                    <!--<td></td>-->
                                </tr>
                                </tfoot>
                            </table>
...
@section('script')
    ...
    <script>
        $(function () {
            ...
            var selectedSegment = '{{ old('segment', $requisition->segment_id) }}';
            ...
            $('body').on('keyup', '.quantity','.approved_quantity', function () {
                calculate();
            });
            $('body').on('keyup', '.approved_quantity', function () {
                calculate();
            });
        ...
        function calculate() {
            ...
            $('.product-item').each(function(i, obj) {
                ...
                totalApproved += parseFloat(approved_quantity);
            });

            $('#total-quantity').html(total);
            $('#total-approved-quantity').html(totalApproved);
        }
