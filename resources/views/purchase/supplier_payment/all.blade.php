                        ...
                        <table id="table" class="table table-bordered table-striped">
                            ...
                            @foreach($suppliers as $supplier)
                                <tr>
                                    ...
                                    <td> {{ number_format($supplier->purchase_order_total, 2) }}</td>
                                    <td> {{ number_format($supplier->purchase_order_paid, 2) }}</td>
                                    <td> {{ number_format($supplier->purchase_order_refund, 2) }}</td>
                                    <td> {{ number_format($supplier->purchase_order_due, 2) }}</td>
                                    <td> {{ number_format($supplier->purchase_discount, 2) }}</td>
                                    <td>
                                        <a class="btn btn-success btn-sm btn-pay" role="button" data-id="{{ $supplier->id }}" data-name="{{ $supplier->name }}">Pay</a>
                                        @if($supplier->purchase_order_refund > 0)
                                            <a class="btn btn-danger btn-sm btn-refund" role="button" data-id="{{ $supplier->id }}" data-name="{{ $supplier->name }}">Refund</a>
                                        @endif
                                        <a href="{{route('supplier_payment_details',['client'=>$supplier->id])}}" class="btn btn-primary btn-sm" >Details</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="modal-form" enctype="multipart/form-data" name="modal-form">
                        ...
                        <div class="form-group">
                            <label>Order</label>
                            <select style="width: 100%" class="form-control select2" id="modal-order" name="order">
